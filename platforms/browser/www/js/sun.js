...
function getJD(dateobj) {
    ...
    */
   	var docmonth = dateobj.getMonth() + 1;
    var docday = dateobj.getDate();
    var docyear = dateobj.getFullYear();
    if (docmonth <= 2) {
        docyear -= 1
        docmonth += 12
    }
...
function calcSunriseSet(rise, JD, latitude, longitude, timezone, dst,dateobj) {
    ...
    */
   	if (response.length > 0)
    	return new Date(dateobj.getFullYear(),dateobj.getMonth(),dateobj.getDate(),response[0],response[1],response[2]);
    else
    	return null;
}
...
function sunTimes(dateobj,lat,lng) {
    var jday = getJD(dateobj)
    var tz = (dateobj.getTimezoneOffset() / 60) * -1;
    var response = {};
    response['sunrise'] = calcSunriseSet(1, jday, lat, lng, tz, null,dateobj)
    response['sunset'] = calcSunriseSet(0, jday, lat, lng, tz, null,dateobj)
    return response;
}